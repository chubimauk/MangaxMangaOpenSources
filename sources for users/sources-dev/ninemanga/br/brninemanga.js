...
module.exports = class BrNineManga extends Source  {

    constructor() {
        super();
        this.baseUrl = 'https://br.ninemanga.com';
    }
    ...
    chapterFromElement(chapterElement, title){
        ...
        if (name.match(/\bch\b/gi)){
           ...
           nam = n[0]
           reg = RegExp(/\b\d+\.?\d?\b/g)
        } else if (name.match(/\bcap\b (\b\d+\.?\d?\b)/gi)){
            var n = name.match(/\bcap\b(.*)/gi)
            nam = n[0]
            reg = RegExp(/\b\d+\.?\d?\b/g)
        ...
        return super.chapter(url, "Brazilian Portuguese", volumeNumber, chapterNumber, name, date_upload, scanlator);
    }
    ...
    mangaDetailsParse(response, $, seriesURL){
        ...
        let artist = author
        let status = $('li a.red',info).first().text().toUpperCase().trim().replace(/EM TRADUÇÃO/gi,"ONGOING").replace(/COMPLETO/gi,"COMPLETED");
        var genres = [];
    ...
    pageListParse(pageListResponse, chapter){
        if(arguments.length == 3){
            ...
            console.log("BrNineManga chainedResponse -- ", requestsForPagesWithImages);
           ...
           var headers = {
            ...
            for(var index = 1; index <= lastPageNumber; index++) {
                let pageURL = `https://br.ninemanga.com${baseChapterPageURL}-${index}.html`;
                requestsForPagesWithImages[`${index}`] = thisReference.jsonBrowserifyRequest(pageURL,null,null,headers,null);
            }
            
            console.log("BrNineManga pageURLs -- ", requestsForPagesWithImages);
    ...
    async fetchLatestManga(page){
        console.log("fetchLatestManga -- BrNineManga");
    ...
    latestUpdatesParse(page, response){
        ...
        $(latestUpdatesSelector).each(function (i, elem) {
            var mangaUpdate = new BrNineManga().latestUpdatesFromElement($(this));
            mangaUpdate.updates = 1;
            json.push(mangaUpdate);
        });
        
        console.log("BrNineManga latest -- ", json);
    ...
    fetchSourceInfo() {
        ...
        sourceInfo.displayInfo.push(super.jsonSourceDisplayInfoTag("language",["Brazilian Portuguese"],null));
        ...
        sourceInfo.displayInfo.push(super.jsonSourceDisplayInfoTag("note",["Requer pedidos infinitos de páginas","Requires Infinite requests for pages"],["#ff1100","#ff1100"]));
        sourceInfo.displayInfo.push(super.jsonSourceDisplayInfoTag("tracker",["No"],[])); //should just be No or Yes
        console.log("BrNineManga sourceInfo -- ", sourceInfo);
        return sourceInfo;
    }
    
    searchMangaRequest(page, query, filters) {
        console.log("BrNineManga filters -- ", filters);
        ...
        if (Object.keys(filters).length === 0) {
            ...
            console.log("attempting to fetch search request for BrNineManga - searchUrl is ", url);
            return url;
        }
        else {
            ...
            console.log("attempting to fetch search request for BrNineManga - searchUrl is ", finsihedur);
            return finsihedur;
        }
    }
    ...
    searchMangaParse(page, response, query, filters){
        ...
        $(searchMangaSelector).each(function (i, elem) {
            json.push(new BrNineManga().searchMangaFromElement($(this)));
        });
        var page = parseInt(page);
        var mangasPage = {};
        mangasPage.mangas = json;
