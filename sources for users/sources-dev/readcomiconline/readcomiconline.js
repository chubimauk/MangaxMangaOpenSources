...
module.exports = class Readcomiconline extends Source  {
    ...
    mangaFromElement(element /*cheerio element --> $(this)*/){
        ...
        var thumbnail_url = titleCheerio('img').first().attr("src");
    ...
    getLastPageNumber(firstPageHtml){ //for popular
        ...
        if (lastNumbersOnly != null && lastNumbersOnly.length > 0){ //protect against results with no paging, there won't be a next page if there is only 1 page
            ...
            if (lastLinkNumbersOnly != null && lastLinkNumbersOnly.length > 0){ //protect against results with no paging, there won't be a next page if there is only 1 page
                ...
                if (bruteLastNumbersOnly != null && bruteLastNumbersOnly.length > 0){ //protect against results with no paging, there won't be a next page if there is only 1 page
    ...
    chapterFromElement(chapterElement, source){
        ...
        var scanlator = ""; //TODO
        var date_upload = $("td").eq(1).first().text();
        //Vol tbd. chapter 60
        var volumeNumber = '';
        var chapterNumber = '';
     ...
     chapterListParse(response, $, seriesURL /*not necessary for readcomiconline but needs to match api*/){ //list of chapter
        ...
        var chapters = [];
         ...
         $(this.chapterListSelector()).each(function (i, chapterElement){
             if(i > 1){ //first 2 are title/space
    ...
    mangaDetailsParse(response, $, seriesURL /*not necessary for readcomiconline but needs to match api*/){ //mangaDetails object
   ...
   //readcomiconline
    ...
    pageListRequest(chapter) {
        ...
        if(chapter.chapter.startsWith('http')){ //readType=1 to get all pages
    ...
    pageListParse(pageListResponse,chapter){
        ...
        //const regex = RegExp(/lstImages\.push\(\"(http.*)\"\)/g);
    ...
    searchMangaRequest(page /*Int*/, query, filters) {
        ...
        if (Object.keys(filters).length === 0) {
            ...
            //addQueryParameter(url, name, value, isFirstParameter) //keyword is the parameter
            ...
            //var url = this.baseUrl + `/AdvanceSearch?page=${page}`;
            ...
            for(var i=0; i<filters.length; i++){
                switch(filters[i].key) {
                  case "genres":
                        ...
                        isFirstParameter = false;
                        //this.getGenresOrderedKeyList().indexOf('bison')
                        break;
                  case "status":
    ...
    searchMangaParse(page /*Int*/, response /*String -- latestUpdatesResponse -- currentPageHtml */, query, filters){
        ...
        var $ = cheerio.load(response);
        var directManga = $('.barTitle',$('.rightBox')).first().text().trim()
        ...
        if (`${directManga}` === 'Cover'){
            ...
            var rank = '0';
            directed.push({name,url,thumbnail,rank})
            var mangasPage = {};
    ...
    async getAll() {
        ...
        var firstPageHtml = await new Readcomiconline().send_request(this.popularMangaRequest(1));
