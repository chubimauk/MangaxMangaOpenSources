...
module.exports = class RuNineManga extends Source {
  constructor() {
    super();
    this.baseUrl = 'https://ru.ninemanga.com';
  }
  ...
  chapterFromElement(chapterElement, title) {
    ...
    if (name.match(/\u0413\u043b\u0430\u0432\u0430 (\b\d+\.?\d?\b)/giu)) {
      var n = name.match(/\u0413\u043b\u0430\u0432\u0430(.*)/giu);
      nam = n[0];
      reg = RegExp(/\b\d+\.?\d?\b/giu);
    } else if (name.match(/\b[1-9] (-.)\b/gi)) {
      var n = name.match(/\b[1-9] (-.)\b(.*)/gi);
      nam = n[0];
    ...
    return super.chapter(url, "Russian", volumeNumber, chapterNumber, name, date_upload, scanlator);
  }
  ...
  mangaDetailsParse(response, $, seriesURL) {
    ...
    let artist = author;
    let status = $('li a.red', info).first().text().toUpperCase().trim().replace(/\u0437\u0430\u0432\u0435\u0440\u0448\u0435\u043d\u043d\u044b\u0439/giu, "COMPLETED");
    var genres = [];
  ...
  pageListParse(pageListResponse, chapter) {
    if (arguments.length == 3) {
      ...
      console.log("RuNineManga chainedResponse -- ", requestsForPagesWithImages);
      ...
      for (var index = 1; index <= lastPageNumber; index++) {
        let pageURL = `https://ru.ninemanga.com${baseChapterPageURL}-${index}.html`;
        requestsForPagesWithImages[`${index}`] = thisReference.jsonBrowserifyRequest(pageURL, null, null, headers, null);
      }

      console.log("RuNineManga pageURLs -- ", requestsForPagesWithImages);
  ...
  async fetchLatestManga(page) {
    console.log("fetchLatestManga -- RuNineManga");
  ...
  latestUpdatesParse(page, response) {
    ...
    $(latestUpdatesSelector).each(function (i, elem) {
      var mangaUpdate = new RuNineManga().latestUpdatesFromElement($(this));
      mangaUpdate.updates = 1;
      json.push(mangaUpdate);
    });
    console.log("RuNineManga latest -- ", json);
    var mangasPage = {};
  ...
  fetchSourceInfo() {
    ...
    sourceInfo.displayInfo.push(super.jsonSourceDisplayInfoTag("language", ["Russian"], null));
    ...
    sourceInfo.displayInfo.push(super.jsonSourceDisplayInfoTag("note", ["Требуется бесконечное количество запросов на страницы", "Requires Infinite requests for pages"], ["#ff1100", "#ff1100"]));
    sourceInfo.displayInfo.push(super.jsonSourceDisplayInfoTag("tracker", ["No"], [])); //should just be No or Yes

    console.log("RuNineManga sourceInfo -- ", sourceInfo);
    return sourceInfo;
  }

  searchMangaRequest(page, query, filters) {
    console.log("RuNineManga filters -- ", filters);
    ...
    if (Object.keys(filters).length === 0) {
      ...
      console.log("attempting to fetch search request for RuNineManga - searchUrl is ", url);
      return url;
    } else {
      ...
      console.log("attempting to fetch search request for RuNineManga - searchUrl is ", finsihedur);
      return finsihedur;
    }
  }
  ...
  searchMangaParse(page, response, query, filters) {
    ...
    $(searchMangaSelector).each(function (i, elem) {
      json.push(new RuNineManga().searchMangaFromElement($(this)));
    });
    var page = parseInt(page);
    var mangasPage = {};
    mangasPage.mangas = json;
  ...
  getGenresList() {
    return {
      ...
      "85": "вампиры",
      "73": "гарем",
      "81": "гендерная интрига",
      "68": "героическое фэнтези",
      "72": "детектив",
      "64": "дзёсэй",
      "62": "додзинси",
      "51": "драма",
      ...
      "54": "психология",
      "61": "романтика",
      "82": "самурайский боевик",
      "55": "сверхъестественное",
      "67": "сёдзё",
      "78": "сёдзё-ай",
      "52": "сёнэн",
      "63": "сёнэн-ай",
