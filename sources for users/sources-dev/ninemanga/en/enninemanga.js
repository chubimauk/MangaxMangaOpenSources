...
module.exports = class EnNineManga extends Source  {

    constructor() {
        super();
        this.baseUrl = 'https://en.ninemanga.com';
    }
    ...
    chapterFromElement(chapterElement, title){
        ...
        } else if (name.match(/\bchapter\b (\b\d+\.?\d?\b)/gi)){
           var n = name.match(/\bchapter\b(.*)/gi)
           nam = n[0]
           reg = RegExp(/\b\d+\.?\d?\b/g)
        } else if(name.toLowerCase().indexOf(title.toLowerCase()) >= 0){
            console.log('has title regex')
    ...
    mangaDetailsParse(response, $, seriesURL){
        ...
        let title =  $('li > span:not([class])',info).text().trim().replace(/ Manga/gi,"")
        let thumbnail = $('img[itemprop="image"]',info).attr('src');
        let author = $('li a[itemprop="author"]',info).text().trim();
        let artist = author
        let status = $('li a.red',info).first().text().toUpperCase().trim();
        var genres = [];
        $('li[itemprop="genre"] a',info).each(function (i, chapterElement){
            var gen = $(chapterElement).text();
            genres.push(gen);
        });
        let description = $('p[itemprop="description"]',info).text().trim().replace(/Summary:\n/gi,"");
    ...
    pageListParse(pageListResponse, chapter){
        if(arguments.length == 3){
            ...
            console.log("EnNineManga chainedResponse -- ", requestsForPagesWithImages);
           ...
           var headers = {
            ...
            for(var index = 1; index <= lastPageNumber; index++) {
                let pageURL = `https://en.ninemanga.com${baseChapterPageURL}-${index}.html`;
                requestsForPagesWithImages[`${index}`] = thisReference.jsonBrowserifyRequest(pageURL,null,null,headers,null);
            }
            
            console.log("EnNineManga pageURLs -- ", requestsForPagesWithImages);
    ...
    async fetchLatestManga(page){
        console.log("fetchLatestManga -- EnNineManga");
    ...
    latestUpdatesParse(page, response){
        ...
        $(latestUpdatesSelector).each(function (i, elem) {
            var mangaUpdate = new EnNineManga().latestUpdatesFromElement($(this));
            mangaUpdate.updates = 1;
            json.push(mangaUpdate);
        });
        
        console.log("EnNineManga latest -- ", json);
    ...
    fetchSourceInfo() {
        ...
        sourceInfo.displayInfo.push(super.jsonSourceDisplayInfoTag("note",["Requires Infinite requests for pages"],["#ff1100"]));
        sourceInfo.displayInfo.push(super.jsonSourceDisplayInfoTag("tracker",["No"],[])); //should just be No or Yes
        console.log("EnNineManga sourceInfo -- ", sourceInfo);
        return sourceInfo;
    }
    
    searchMangaRequest(page, query, filters) {
        console.log("EnNineManga filters -- ", filters);
        ...
        if (Object.keys(filters).length === 0) {
            ...
            console.log("attempting to fetch search request for EnNineManga - searchUrl is ", url);
            return url;
        }
        else {
            ...
            console.log("attempting to fetch search request for EnNineManga - searchUrl is ", finsihedur);
            return finsihedur;
        }
    }
    ...
    searchMangaParse(page, response, query, filters){
        ...
        $(searchMangaSelector).each(function (i, elem) {
            json.push(new EnNineManga().searchMangaFromElement($(this)));
        });
        var page = parseInt(page);
        var mangasPage = {};
        mangasPage.mangas = json;
