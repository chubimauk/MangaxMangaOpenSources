...
module.exports = class Manganelo extends Source {
  ...
  chapterFromElement(chapterElement, source) {
    ...
    var date_upload = $('span').last().text(); //Vol tbd. chapter 60
  ...
  /*not necessary for manganelo but needs to match api*/
    ...
    let views = 0;
    let lastUpdate = '';
    let hentai = false; //let tagSections: TagSection[] = [createTagSection({ id: '0', label: 'genres', tags: [] })]
    ...
    for (let row of $('tr', table).toArray()) {
      if ($(row).find('.info-alternative').length > 0) {
        ...
        for (let elem of elems) {
          let text = $(elem).text(); //let id = $(elem).attr('href')?.split('/').pop()?.split('-').pop() || '';
    ...
    let description = $('.panel-story-info-description', panel).text(); //id, titles, image, raiting, status, artist, author, tags, views, follows, lastUpdate, desc, hentai
    ...
    return this.mangaDetails(title, thumbnail, description, author, artist, status, genres); //based on Tachiyomi MangaBox -- author, genre not working
  ...
  /*String -- latestUpdatesResponse -- currentPageHtml */
    ...
    var $ = cheerio.load(response); //DEFAULT FROM PARSEDHTTPSOURCE.KT
    ...
    console.log("mangenlo latest -- ", json); //var lastPageNumber = await this.getLastPageNumber(response); //this should work on every page for this source

    var mangasPage = {};
    mangasPage.mangas = json; //mangasPage.hasNextPage = lastPageNumber > page;
  ...
  , query, filters) {
    ...
    if (Object.keys(filters).length === 0) {
      ...
      var genreInclude = "";
      var genreExclude = ""; //getGenresList
      ////////////////////
  ...
  async getAll() {
    ...
    var firstPageHtml = await new Manganelo().send_request(this.popularMangaRequest(1)); //this.getAllPageNumbers(firstPageHtml);
