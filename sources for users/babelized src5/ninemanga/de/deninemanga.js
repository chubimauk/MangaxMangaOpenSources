...
module.exports = class DeNineManga extends Source {
  constructor() {
    super();
    this.baseUrl = 'https://de.ninemanga.com';
  }
  ...
  chapterFromElement(chapterElement, title) {
    ...
    } else if (name.match(/\bkapitel\b (\b\d+\.?\d?\b)/gi)) {
      var n = name.match(/\bkapitel\b(.*)/gi);
      nam = n[0];
      reg = RegExp(/\b\d+\.?\d?\b/g);
    } else if (name.match(/\bep\b/gi)) {
      var n = name.match(/\bep\b(.*)/gi);
      nam = n[0];
      reg = RegExp(/\b\d+\.?\d?\b/g);
    ...
    return super.chapter(url, "German", volumeNumber, chapterNumber, name, date_upload, scanlator);
  }
  ...
  mangaDetailsParse(response, $, seriesURL) {
    ...
    let artist = author;
    let status = $('li a.red', info).first().text().toUpperCase().trim().replace(/Laufende/gi, "ONGOING").replace(/Abgeschlossen/gi, "COMPLETED");
    var genres = [];
  ...
  pageListParse(pageListResponse, chapter) {
    if (arguments.length == 3) {
      ...
      console.log("DeNineManga chainedResponse -- ", requestsForPagesWithImages);
      ...
      for (var index = 1; index <= lastPageNumber; index++) {
        let pageURL = `https://de.ninemanga.com${baseChapterPageURL}-${index}.html`;
        requestsForPagesWithImages[`${index}`] = thisReference.jsonBrowserifyRequest(pageURL, null, null, headers, null);
      }

      console.log("DeNineManga pageURLs -- ", requestsForPagesWithImages);
  ...
  async fetchLatestManga(page) {
    console.log("fetchLatestManga -- DeNineManga");
  ...
  latestUpdatesParse(page, response) {
    ...
    $(latestUpdatesSelector).each(function (i, elem) {
      var mangaUpdate = new DeNineManga().latestUpdatesFromElement($(this));
      mangaUpdate.updates = 1;
      json.push(mangaUpdate);
    });
    console.log("DeNineManga latest -- ", json);
    var mangasPage = {};
  ...
  fetchSourceInfo() {
    ...
    sourceInfo.displayInfo.push(super.jsonSourceDisplayInfoTag("language", ["German"], null));
    ...
    sourceInfo.displayInfo.push(super.jsonSourceDisplayInfoTag("note", ["Erfordert unendliche Anfragen für Seiten", "Requires Infinite requests for pages"], ["#ff1100", "#ff1100"]));
    sourceInfo.displayInfo.push(super.jsonSourceDisplayInfoTag("tracker", ["No"], [])); //should just be No or Yes

    console.log("DeNineManga sourceInfo -- ", sourceInfo);
    return sourceInfo;
  }

  searchMangaRequest(page, query, filters) {
    console.log("DeNineManga filters -- ", filters);
    ...
    if (Object.keys(filters).length === 0) {
      ...
      console.log("attempting to fetch search request for DeNineManga - searchUrl is ", url);
      return url;
    } else {
      ...
      console.log("attempting to fetch search request for DeNineManga - searchUrl is ", finsihedur);
      return finsihedur;
    }
  }
  ...
  searchMangaParse(page, response, query, filters) {
    ...
    $(searchMangaSelector).each(function (i, elem) {
      json.push(new DeNineManga().searchMangaFromElement($(this)));
    });
    var page = parseInt(page);
    var mangasPage = {};
    mangasPage.mangas = json;
